...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($result) > 0) {
        $message = "<div class='alert alert-danger' style='display:block;'>❌ Email is already registered!</div>";
    } else {
        $checkAnyUser = "SELECT id FROM users LIMIT 1";
        $userCheckResult = mysqli_query($conn, $checkAnyUser);
        $role = (mysqli_num_rows($userCheckResult) == 0) ? 'admin' : 'customer';

        $query = "INSERT INTO users (fullname, email, password, role) VALUES ('$fullname', '$email', '$password', '$role')";
        if (mysqli_query($conn, $query)) {
            $message = "<div class='alert alert-success' style='display:block;'>✅ Account created successfully! <a href='login.php'>Login here</a></div>";
        } else {
            $message = "<div class='alert alert-danger' style='display:block;'>❌ Error: " . mysqli_error($conn) . "</div>";
        }
    }
}
?>
...
<head>
    ...
    <style>
        ...
        .alert-danger { background-color: #ffebee; color: #c62828; padding: 10px; border-radius: 4px; margin-bottom: 15px; }
        .alert-success { background-color: #e8f5e9; color: #2e7d32; padding: 10px; border-radius: 4px; margin-bottom: 15px; }
    </style>
</head>
<body>
...
<div class="form-container">
    ...
    <form id="regForm" action="register.php" method="POST" onsubmit="return validateRegister(event)">
